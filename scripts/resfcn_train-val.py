...
from models.fcn import residual_fcn_3d
from custom_loss import sparse_dice, sparse_dice_plus_xent_loss, dense_dice, dense_cross_entropy 
...
def model_fn(features, labels, mode, params):
    ...
    # Model Definition
    model_output_ops = residual_fcn_3d(
        inputs                   = features['x'],
        num_res_units            = 3,
    ...
    # Loss Function
    if (LOSS_FUNC=='SPARSE_CE'):
        loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=model_output_ops['logits'], labels=labels['y']))
    elif (LOSS_FUNC=='SPARSE_DSC'):
        loss = sparse_dice(prediction=model_output_ops['logits'], ground_truth=labels['y'])
    elif (LOSS_FUNC=='SPARSE_DSC_CE'):
        loss = tf.reduce_mean(sparse_dice_plus_xent_loss(prediction=model_output_ops['logits'], ground_truth=labels['y']))
    elif (LOSS_FUNC=='DENSE_DSC'):
        loss = dense_dice(prediction=model_output_ops['logits'], ground_truth=labels['y'])
    elif (LOSS_FUNC=='DENSE_DSC_CE'):
        loss = tf.reduce_mean(dense_cross_entropy(prediction=model_output_ops['logits'], ground_truth=tf.one_hot(labels['y'],depth=NUM_CLASSES)) 
               + dense_dice(prediction=model_output_ops['logits'], ground_truth=tf.one_hot(labels['y'],depth=NUM_CLASSES)))
    ...
    image_summaries = {}
    image_summaries['MRI_Patch']   = features['x'][0, 25, :, :, 0]
    image_summaries['Predictions'] = tf.cast(model_output_ops['y_'], tf.float32)[0, 25, :, :]
...
def train(args):
    ...
    reader_patch_shapes = {'features': {'x': reader_params['patch_size'] + [NUM_CHANNELS,]},
                           'labels':   {'y': reader_params['patch_size']}}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path',   '-p', default='/home/cad1/anindo/neuro/model_weights/test/ensemble/resfcn64-350E/')
    parser.add_argument('--train_csv',    '-t', default='/home/cad1/anindo/neuro/feed/Full-Training-Fold-O.csv')
    parser.add_argument('--val_csv',      '-v', default='/home/cad1/anindo/neuro/feed/Validation-Fold-O.csv')
